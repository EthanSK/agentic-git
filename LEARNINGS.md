...
**Fix:** Agentic Git v2.0.0 moves the manifest, command IDs, settings, runtime strings, tests, README, changelog, and current learning references to `agentic-git`; Ethan's explicit VS Code settings/keybindings are migrated without restarting or installing anything. The GitHub repository is renamed so GitHub preserves redirects, the new `EthanSK.agentic-git` Marketplace identity is published, and the 1.x listing is retained instead of being unpublished. VS Code's official process has no `vsce` or publisher-portal deprecation action: request the old-to-new migration mapping in [`microsoft/vscode-discussions` #1](https://github.com/microsoft/vscode-discussions/discussions/1), then wait for Microsoft to apply it. This launch's request is [discussion comment 17641159](https://github.com/microsoft/vscode-discussions/discussions/1#discussioncomment-17641159). The caret-owned v1.2.26 behavior is preserved and explained in `docs/navigation-behavior.md`.
**Guard:** A pre-release zero-stale-identity search must pass across tracked current files and Ethan's active VS Code settings/keybindings. Compile, lint, all 35 real isolated-host E2Es, a production VSIX identity/tree inspection, GitHub old/new URL verification, and a version-specific Marketplace package download are required. The Marketplace `vspackage` response can be HTTP content-encoded as gzip; decode it (for example with `curl --compressed`) before comparing its VSIX hash and archive tree. Do not claim deprecation is active until Marketplace flags or VS Code's migration UI confirm Microsoft applied the request. Never install, reload, or restart Ethan's normal VS Code during this release.
---
